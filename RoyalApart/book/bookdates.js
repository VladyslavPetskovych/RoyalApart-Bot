...
function handleDaySelection(chatId, selectedDay) {
  ...
  log(`Selected date: ${selectedDate.toISOString()}`);
  ...
  log(`Displayed date string: ${displayedDateString}`);
...
function qOptionsDates(month, currentDay) {
  log(
    `Generating options for dates, month: ${month}, currentDay: ${currentDay}`
  );
  return {
...
bot.on("callback_query", async (msg) => {
  ...
  log(`Callback query received, data: ${data}, chatId: ${chatId}`);
